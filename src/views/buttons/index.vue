...
const form=reactive({buttonId:'',label:'',groupId:'',priority:99,enabled:true});let eid=''
onMounted(async()=>{try{buttons.value=await listButtons()}catch(_){}})
function openAdd(){isEdit.value=false;Object.assign(form,{buttonId:'',label:'',groupId:'',priority:99,enabled:true});visible.value=true}
function openEdit(row){isEdit.value=true;eid=row.buttonId;Object.assign(form,{buttonId:row.buttonId,label:row.label,groupId:row.groupId,priority:row.priority||99,enabled:row.enabled});visible.value=true}
async function onSave(){try{const d={label:form.label,groupId:form.groupId,priority:form.priority,enabled:form.enabled};if(isEdit.value)await updateButton(eid,d);else await createButton({...d,buttonId:form.buttonId});ElMessage.success('OK');visible.value=false;buttons.value=await listButtons()}catch(_){}}
</script>
